import { User, Release} from './../../the-lab/malv-data.service';
...
export const attachmentDataLayoutFactory = (amount: number)=>{
	const attachmentDataLayouts: AttachmentDataLayout[] = [];
	for (let i = 0; i < amount; i++) {
		let c: AttachmentDataLayout = {
			filename: MOCK_FILENAMES[randomArraySelector(MOCK_NAMES.length)],
			sha1: String(Math.round(Math.random()*100)),
			attachmentType:ATTACHMENT_TYPE[randomArraySelector(ATTACHMENT_TYPE.length)],
			createdTeam:  'Team '+String(Math.round(Math.random()*10)),
			createdOn: MOCK_CREATED_ON[randomArraySelector(MOCK_CREATED_ON.length)],
			checkedTeam: 'Team '+String(Math.round(Math.random()*10)),
			checkedComment: 'We checked and decided to '+ ATTACHMENT_CHECKED_STATES[randomArraySelector(ATTACHMENT_CHECKED_STATES.length)],
			checkedOn: MOCK_CREATED_ON[randomArraySelector(MOCK_CREATED_ON.length)],
			checkStatus: ATTACHMENT_CHECKED_STATES[randomArraySelector(ATTACHMENT_CHECKED_STATES.length)],
			_embedded: null,
			_links: null
		}
		attachmentDataLayouts.push(c);
	}
	return attachmentDataLayouts;
}
...
export enum AttachmentCheckedState {
	NOTCHECKED = 'NOT_CHECKED',
	ACCEPTED = 'ACCEPTED',
	REJECTED = 'REJECTED'
}
export const ATTACHMENT_CHECKED_STATES = [
	AttachmentCheckedState.NOTCHECKED,
	AttachmentCheckedState.ACCEPTED,
	AttachmentCheckedState.REJECTED
]
...
export interface AttachmentDataLayout {
	filename: string;
	sha1: string;
	...
	checkedOn: string;
	checkStatus: AttachmentCheckedState;
	_embedded: AttachmentEmbedded;
	_links: Links
}

const MOCK_FILENAMES =['description.doc','report.xml','binaries.txt','screenshot.jpg'];
